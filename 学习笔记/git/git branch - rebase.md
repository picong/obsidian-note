### rebase 和 merge的区别如下图![[学习笔记/git/attachments/Pasted image 20230817161647.png]]]merge/rebase之前的提交记录
![[学习笔记/git/attachments/Pasted image 20230817161647.png]]]merge之后的提交记录
![[学习笔记/git/attachments/Pasted image 20230817162005.png]]]rebase之后的提交记录
### 基本的rebase操作
```shell
...
```
以上命令的意思是：“取出client分支，找出它从server分支分叉之后的补丁，然后把这些补丁在master分支上重放一遍，让client看起来像直接基于master修改一样”。![[学习笔记/git/attachments/Pasted image 20230817174440.png]]]从一个主题分支再分出一个主题分支的提交历史![[学习笔记/git/attachments/Pasted image 20230817174515.png]]]截取主题分支上的另一个主题分支，然后rebase到其他分支
...
### rebase的风险
rebase操作的实质是丢弃一些现有的提交，然后相应地新建一些内容一样但实际上不同的提交。如果你已经将提交推送至某些仓库，而其他人也已经从该仓库拉取提交并进行了后续工作，此时，如果你用`git rebase` 命令重新整理了提交并再次推送，你的同伴将不得不再次将他们手头的工作与你的提交进行整合，如果接下来你还要拉取并整合他们修改过的提交，事情将会变成一团糟。或者你已经rebase之后，其他人又强制修改了你所基于的那些提交，并强制推送了，这时候几有可能出现两次一样的提交历史，如果你又pull了他们的代码的话。![[学习笔记/git/attachments/Pasted image 20230817184713.png]]]
### 用rebase解决rebase问题
